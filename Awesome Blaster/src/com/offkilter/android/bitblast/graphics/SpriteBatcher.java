...
public class SpriteBatcher implements Renderer
{
  ...
  public void onSurfaceCreated(GL10 gl, EGLConfig config)
  {
    ...
    {
      addTexture(gl, BitmapFactory.decodeResource(resources, bitmapIds[i]),
          textureIds[i]);
    }
  }
      ...
      Rect hotRect, int angle, float scale, float alpha)
  {
    ...
    draw(gl, bitmapId, src, drawX, drawY, hotRect, angle, scale, scale, alpha);
  }
      ...
      Rect hotRect, int angle, float sizeX, float sizeY, float alpha)
  {
    ...
    for (int i = 0; i < bitmapIds.length; i++)
    {
      if (bitmapId == bitmapIds[i])
      {
        spriteData.get(i).addSprite(src, drawX, drawY, hotRect, angle, sizeX,
            sizeY, alpha);
        return;
      }
    }
  ...
  private class SpriteData
  {
    ...
    public void addSprite(Rect src, Rect dst)
    {
      ...
      // TEXTURE COORDS
      float[] srcX =
      { src.left, src.left, src.right, src.right };
      float[] srcY =
        ...
        int angle, float sizeX, float sizeY, float alpha)
    {
      ...
      // Coordinates before rotation
      float[] hotX =
      { hotRect.left, hotRect.left, hotRect.right, hotRect.right };
      float[] hotY =
      { hotRect.top, hotRect.bottom, hotRect.bottom, hotRect.top };
      for (int i = 0; i < 4; i++)
      {
